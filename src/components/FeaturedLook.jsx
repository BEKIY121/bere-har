...
const FeaturedLook = () => {
  return (
    <section style={{ backgroundColor: 'var(--bg-cream)', position: 'relative', overflow: 'hidden', padding: 'clamp(3rem, 6vw, 6rem) 0' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }}
          ...
          <div>
            ...
            <h2 style={{ fontSize: 'clamp(2.2rem, 4vw, 3.2rem)', fontFamily: 'Playfair Display, serif', fontWeight: '400', lineHeight: 1 }}>The Featured Look</h2>
          </div>
          <a href="/shop" style={{ fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.2em', borderBottom: '1px solid var(--text-dark)', paddingBottom: '0.3rem' }}>
            View All →
          </a>
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: '1.5rem', alignItems: 'end' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} style={{ position: 'relative' }}>
            <div style={{ overflow: 'hidden', height: 'clamp(400px, 50vh, 550px)' }}>
            ...
            <div style={{ position: 'absolute', bottom: '1.5rem', left: '1rem' }}>
              <span style={{ backgroundColor: 'var(--bg-cream)', padding: '0.4rem 0.8rem', fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.15em' }}>New Season</span>
            </div>
          </motion.div>

          {/* Right Two Stacked */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.15 }} style={{ overflow: 'hidden', height: 'clamp(200px, 25vh, 260px)' }}>
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.25 }}
              style={{ padding: '1rem', border: '1px solid rgba(26,26,26,0.1)', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <p style={{ fontSize: '0.55rem', textTransform: 'uppercase', letterSpacing: '0.2em', color: 'var(--text-muted)' }}>Artisan Note</p>
              <p className="script-font" style={{ fontSize: '1.5rem' }}>Made by hand.</p>
              <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>Individually crafted by our atelier team in Asmara.</p>
            </motion.div>
          </div>

          {/* Third Column */}
          <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.3 }} style={{ overflow: 'hidden', height: 'clamp(300px, 40vh, 420px)', marginTop: '2rem' }}>
